...
//----------------------------------------------------FILE ENCRYPTION, DECRIPTION, SIGNING AND VERIFICATION--------------------------------------------//
...
function encrypteAndDecyrpt(plaintext, privateKey, publicKey){ 
...
router.post('/sign',(req,res)=>{
  ...
  for ( file of files ) {
      const sign = crypto.createSign('SHA256');
      let data = fs.readFileSync(`${__dirname}/uploadedFiles/${file}`, "utf8");
      encrypteAndDecyrpt(data, privateKey, publicKey); 
      sign.update(data);
      sign.end();
